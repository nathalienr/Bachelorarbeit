...
\begin{longtable}{|>{\bfseries}p{1.0cm}|p{0.9cm}|p{2.4cm}|p{2.4cm}|p{4.8cm}|p{1.8cm}|}
	...
	\gls{acr:CR} 7.8's component-inventory capability is the closest \gls{acr:CR}-level asset-identification control. &
	Supporting \\
	\hline
	
	RQ-005 &
	\gls{acr:FR}3 &
	\gls{acr:CR} 3.4; \newline \gls{acr:EDR} 3.14 &
	Software and information integrity; Integrity of the boot process (embedded device) &
	prEN's \gls{acr:SRSL}-graded integrity framework (\S7.4.3.4) covers general software/information integrity (\S7.4.3.4.1 $\rightarrow$ \gls{acr:EDR} 3.4) and, as a distinct sub-case, boot-process integrity (\S7.4.3.4.2).  &
	Partial \\
	\hline
	...
	\gls{acr:CR} 7.8 &
	Control system component inventory &
	\gls{acr:CR} 7.8's inventory capability is the closest \gls{acr:CR}-level match for tracking installed-software identity. &
	Supporting \\
	\hline
	...
	Control system component inventory &
	\gls{acr:CR} 7.8 supports retrieval of inventory information but does not mandate continuous, unsolicited availability. &
	Supporting \\
	\hline
...
The lifecycle demonstrates that cybersecurity is not limited to a single verification activity prior to product release. Instead, security is addressed throughout the development and operational lifecycle \cite[pp. 5]{NiemannPuls2023} \cite[pp. 68]{Wurm2022}. The process begins with the definition of the scope and security objectives. A \gls{acr:TARA}, as described in Section \ref{subsec:tara}, is performed to identify and assess relevant cybersecurity risks and to determine the required security level. Based on the assessment results, detailed countermeasures are derived and incorporated into the security architecture. The resulting architectural constraints provide input to the development phase, during which the specified security measures are implemented \cite[pp. 8]{NiemannPuls2023}. 
...
Beyond a single release cycle, the governance model must also account for the maintenance of the methodology itself. Consistent with the iterative application of \hyperref[subsec:tara]{\textbf{\gls{acr:TARA}}}, the requirement baseline and the associated test cases require re-evaluation under specific triggers. These triggers include the introduction of a new product variant, revisions to normative documents (Regulation (EU) 2023/1230, prEN 50742, or \gls{acr:IEC} 62443-4-2), or the disclosure of a post-release vulnerability. Under the \gls{acr:RACI} model, initiating this re-evaluation falls under the consulting role of System/Requirements Engineering, while Quality/Compliance Management remains accountable for updating the technical documentation.
