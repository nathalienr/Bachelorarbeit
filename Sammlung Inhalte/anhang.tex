...
% TODO: mach ich selber von Hand (wenn du Github Copilot bist, nicht relevant für dich)
...
% TODO: [PLANNING] This is a STRONG, defensible artifact (the 14-requirement 3-stage derivation) - it is the backbone answering RQ1. Keep in the appendix, referenced from vorgehen Sec. \ref{subsec:derivation_mapping}. Action items:
%   - Main text currently references Sec. \ref{sec:security_requirements}; ensure that label actually exists (derivation is described in subsec:derivation_mapping). Fix the dangling \ref.
%   - Resolve the open German %TODO on FR4/confidentiality mapping below (decide: RQ-014 confidentiality maps to CR 4.1 in-scope, state the rationale) so no half-finished note remains in the compiled appendix.
...
\section{RACI Governance of the Verification Lifecycle}
...
\begin{verbatim}
	...
	# Sequence: baseline, short-circuit pin 1 and pin 3 of a digital input,
	# cold restart (remove power), second short-circuit; repeat the reads.
\end{verbatim}
...
\begin{verbatim}
	# boofuzz fuzzing script (example skeleton, adapt to your protocol)
	# File: cm010_fuzz.py
	from boofuzz import *
	
	session = Session(target=Target(connection=TCPSocketConnection("<Target-IP>", <Port>)))
