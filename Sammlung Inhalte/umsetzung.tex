...
\begin{itemize}
	\item \textbf{Asset:} the entries of Table \ref{tab:assets} affected by the requirement. A requirement typically protects more than one asset, so several letters can appear in one row.
...
Table \ref{tab:cm_applicability} records, for each of the ten company tests, whether the interface or precondition that the tool assumes is present on the \gls{acr:SRIO} in principle. This determination is made before execution and is a property of the device and the topology, not of a completed test run; whether an applicable tool was actually executed, and what it found, is reported in Section \ref{sec:executed_tests}. CM-002 requires a second network zone, which the flat segment does not provide. CM-003 assumes an authentication endpoint, which the device does not expose. CM-006 assumes an operating-system shell, which the embedded device does not provide. CM-005 assumes a dedicated \gls{acr:TLS} endpoint on \gls{acr:TCP} port 443. The device's diagnostic service is documented as plain \gls{acr:HTTP} on port 80 and provides no separate \gls{acr:HTTPS}/443 endpoint.
...
The two-level test catalogue derived in Chapter \ref{chap:methodology} defines abstract test scenarios for all fourteen requirements, including the tests that depend on the passive-observation variant \gls{acr:NV2} and the in-path variant \gls{acr:NV3}. Only a subset of these was executed in the practical phase: the applicable company tests and the requirement-oriented tests that the NV1 position supports. This section reports the seven security categories of Table \ref{tab:security_categories} in the same order as Table \ref{tab:cm_tc_map}, so that the abstract mapping, the selected method, and the executed result can be read together. The tests that require NV2 or NV3 are described as designed but not executed. Every reported result is limited to what the stored evidence shows. A tool execution is not treated as requirement verification, and an expected outcome from a test specification is not presented as an observed result.
...
Section 1.2.1 of the Regulatory requires that the control system withstand reasonably foreseeable malicious attempts that could lead to a hazardous situation (RQ-010), examined here specifically under malformed input rather than under volume load. This category therefore examines whether the \gls{acr:HTTP} service maintains its specified behaviour when it receives malformed requests, and whether any resulting disruption remains confined to the non-safety interface. boofuzz (CM-010) fuzzes the request line of an \gls{acr:HTTP} GET request against the \gls{acr:IoT}-Core diagnostic service under \gls{acr:NV1} (Table \ref{tab:cm_tc_map}); the fieldbus-discovery and safety-frame fuzzing scenarios require the \gls{acr:NV2} or \gls{acr:NV3} position and were designed but not executed. The activity reported below shows a temporary service disruption and an unstored fault-indication observation; neither, by itself, supports a Pass or Fail verdict for RQ-010.
