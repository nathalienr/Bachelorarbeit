...
% TODO: [PLANNING - Implementation and Results | HONESTY IS THE PRIORITY HERE]
% Purpose: apply the methodology to the SUT and report ONLY evidence-backed outcomes. The chapter already follows the evidence-first principle well - keep it.
% STATUS SEPARATION (the user's core constraint): every activity must be tagged as one of
%   [EXECUTED-EVIDENCED] backed by a file in Tests_BA/ ; [SAMPLE] a one-off spot check ; [DESIGNED-NOT-EXECUTED] NV2/NV3 or missing evidence.
% CROSS-CHECK EVERY NUMBER against the stored artifact before submission (evidence DOES exist for the NV1 sample - verify the exact figures come from these files, do not paraphrase from memory):
%   - Nmap 80/tcp, 161/udp -> cm001_srio_tcp/udp, cm_rq001-02_tcp_srio.*  ; Nikto -> cm_rq001-02_srio.html.htm
%   - inventory reads -> cm_rq004-01_inventory.txt, cm_rq006-01_installed.txt, cm_rq007-01_swrev.txt, cm_rq013-01_*
%   - SSLscan -> cm_rq006-03_sslscan.txt / cm005_srio.txt / cm_rq014-02_sslscan.txt
%   - error-log + systick sequence (18329/44979/27509/49889; 0/2/0/1 entries) -> cm_rq003-04_systick_0x.txt + cm_rq003-04_log_0x.json AND cm_rq003-01_errorlog_before/after.json. CONFIRM these exact values match the JSON/txt.
%   - Siege 53-361 tx / 65-100 failed -> cmrq010_siege.log + cmrq010_3connections_siege.log (+ terminal txt). CONFIRM ranges.
%   - connection-hold 200 while slots held -> rq007-03_evidence/
%   - hping3 >12M packets, macof -> which stored file? (Notizen mentions LED fault, but locate the terminal capture; if none, downgrade to [SAMPLE, LED observed, no packet-count artifact] and remove the '12 million' figure unless a log backs it).
%   - boofuzz HTTP unreachable ~40s, code 0x1000 -> boofuzz-results/ + cm010_fuzz_output.log. CONFIRM the 40s and the fault code.
%   - write probe bad-request on 3 nodes -> cm_rq011-01_*_version.txt
%   - ZAP -> cm004_srio_report.html
% Do NOT present any NV2/NV3 scenario, TIA I&M read, iParCRC test, or firmware test as performed - none were executed. The text already handles most of this; verify no expected-result wording slipped in as observed.
% Structure check vs university req (results must be clearly a RESULTS chapter): good. Consider a short 'Threats to validity of the practical results' paragraph near the end (single SUT, NV1 only, scanner findings unvalidated) - some of this is in Zusammenfassung; a brief version here strengthens the results chapter.
...
The System Under Test is the Safe Remote I/O (SRIO), a fieldbus-based safety component that exchanges safety-related input and output data with a higher-level safety controller over PROFINET and PROFIsafe. The evaluated unit identified itself over the IoT-Core interface as product \texttt{ifm-AL400S}, hardware version 1.4.0.2, hardware revision AA, and software revision 1.1.4.888. All reported results refer exclusively to this configuration.

Following the asset classification of Section \ref{subsec:assets_attack_surfaces}, the SRIO is decomposed into hardware, software, and data assets. For the practical assessment, the externally accessible attack surface was divided into the HTTP-based IoT-Core diagnostic interface, the PROFINET and PROFIsafe communication path, the DCP configuration functions, and the firmware update mechanism. Only the interfaces accessible from the implemented NV1 position were exercised. Internal debug and inter-processor interfaces, such as the SysCom link between the COM and SCPU modules and the JTAG interface, are included in the asset model but were not accessible within the practical test scope.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{l l c X}
		...
		Company laptop & 192.168.0.7 & 4 & Engineering station providing TIA Portal for planned identification and maintenance reads \\
		\bottomrule
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{l X X X l}
		...
		RQ-005 & Safety firmware & Integrity protection & Firmware update service & NV1 or physical \\ \addlinespace
		RQ-010 & Device availability & Availability and resilience & HTTP and safety communication & NV1 and NV2 \\ \addlinespace
		RQ-011 & Safety configuration & Authorisation & Configuration interfaces & NV1 or engineering tool \\
		\bottomrule
	\end{tabularx}
...
% TODO: [PLANNING] Good category structure. Keep the clean split between what NV1 supports (executed) and NV2/NV3 (designed). For every 'designed but not executed' scenario, keep the wording in future/subjunctive so it can never be misread as a result.
...
\subsection{Attack Surface and Identification}
...
Three company tests could not be applied to the device, or were blocked by the environment. CM-002 requires a second zone and a conduit model, which the flat laboratory segment does not provide. CM-006 requires an operating-system shell, which the embedded device does not expose. CM-008 could not be operated, because the GVM/OpenVAS scanner could not be brought into a working state within the laboratory setup and the available time. These outcomes are recorded as applicability or environment findings. They are meaningful methodological results, because they show why an applicability assessment is required before execution. They do not indicate a product defect, and they do not support a positive or a negative requirement verdict.
...
This section relates the executed activities to the derived requirements. It uses the verdict scale of Section \ref{subsec:verdicts}. All activities were run under the network variant NV1. A verdict is assigned only where a stored artifact supports it. The execution status of a test and its verdict are distinct: a test can be executed and evidenced yet still leave the requirement inconclusive, and a definitive FAIL can result from an evidenced device deviation even though the test itself succeeded. Where the evidence documents such a deviation, the requirement is assessed as FAIL, because the device does not meet the obligation while the test yields a definitive result.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{l l l X}
		...
		RQ-003 & intervention evidence and time attribution & FAIL & no absolute timestamp, no actor identity \\
		...
		RQ-012 & cold-start deletion & FAIL & log cleared by a cold restart \\
...
The strongest evidence concerns the intervention log. The device records connection-loss events, but the entries carry only a relative system tick, and the log content is cleared by a cold restart. The evidence therefore supports the conclusion that the device deviates from the time-attribution obligation behind RQ-003 and from the retention obligation behind RQ-012. The identification tests support RQ-004, RQ-006, and RQ-007 at product granularity, but not at component granularity, because no software bill of materials is exposed. The robustness test and the load test leave RQ-010 inconclusive for the hazardous-situation clause, because the safety channel was not observed. The write probe shows only that three specific write requests were not accepted; because the device answered with a bad-request response, it does not establish an access-control decision, so RQ-011 remains inconclusive. The remaining requirements are not assessed, because the corresponding configuration, firmware, and safety-channel evidence is not available under NV1.


\subsection{Coverage of the Practical Phase}
\label{subsec:metrics_application}
The assessment model of Section \ref{sec:evaluation_metrics} is qualitative, so the results are summarised as direct counts rather than as ratios. Fourteen requirements were derived from the regulation. Ten company tests were assessed for applicability, of which five were applicable, four were not applicable to the device or the topology, and one was blocked in the laboratory. Fifteen test activities were at least partially executed under the NV1 position. Two requirements, RQ-003 and RQ-012, were assessed as FAIL on the basis of an evidenced device deviation. The remaining executed activities produced PARTIAL or inconclusive outcomes, and several requirements were not assessed, because the corresponding configuration, firmware, or safety-channel evidence was not collected.
% TODO: reconcile these counts exactly with the final Table \ref{tab:req_verification} before submission.

Almost every executed activity is backed by stored raw evidence, including the Nmap and Nikto output, the SSLscan output, the inventory reads, the error-log and system-tick reads, the Siege summaries, the connection-hold output, the write-probe responses, the boofuzz session databases, and the OWASP ZAP report. The evidence base is therefore largely raw and reproducible. The small number of definitive verdicts reflects the deliberately bounded scope of the practical phase rather than a deficiency of the method. The assessment becomes fuller once the outstanding NV2, NV3, firmware, and TIA-based activities are executed.
...
The results are subject to threats to validity that must be stated explicitly, following the distinction between construct, internal, and external validity and reliability.

\textbf{Construct validity} concerns whether a tool actually measures the intended security property. Several observations are indirect. The Nikto Spring Boot Actuator finding is a probable false positive of a generic web scanner. A bad-request response to a write probe does not distinguish input validation from an authorisation decision. The SSLscan run on port 80 confirms only the absence of a TLS endpoint, not a confidentiality property of the transmitted data. These findings are therefore reported as observations rather than as verified security properties.

\textbf{Internal validity} concerns whether an observed effect is caused by the stated input. The fuzzing disruption could not be attributed to a single malformed request, the device state may have been influenced by preceding activities, the activities were not repeated, and the flood observation rests on an LED fault indication without an independent measurement of the safety channel.

\textbf{External validity} concerns generalisation. The evaluation covers a single device, a single firmware version, one topology, and only the NV1 position. The results therefore do not generalise to other gateways, fieldbuses, or configurations.

\textbf{Reliability} concerns repeatability. It is supported by the stored raw evidence, the documented commands in Appendix \ref{app:tool_commands}, and the defined acceptance criteria, so that the executed activities, in particular the logging test, can be repeated. The scenarios that were not executed cannot yet be reproduced and remain open for a future test campaign.
