...
Building upon the theoretical foundations established in Chapter \ref{chap:fundamentals}, this chapter develops the security testing methodology used in this thesis. The methodology translates the machine-level cybersecurity obligations of Regulation (EU) 2023/1230 into derived, component-level requirements and a traceable, repeatable engineering procedure. The regulatory allocation underlying this translation is defined in Section \ref{sec:regulatory_applicability}. The methodology targets the physical IT/OT interface of a safety-related industrial gateway.
...
% Rename the fourteen security requirements RQ-0xx -> SR-0xx across ALL chapters,
% appendices, tables and \label keys in one coordinated pass. Add "SR Security Requirement" to the abbreviations list.
\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Safety priority:} The safety function must not be degraded by testing. A cybersecurity measure or a test must never introduce impermissible latency into the safety communication \cite[p. 14]{prEN50742_2025}. Empirical verification of this constraint requires observation of the safety channel, as provided by the NV2 position in Section \ref{subsec:nv2_passive_observation}. Under NV1, the constraint is maintained procedurally through the isolated laboratory and the exclusion of production systems rather than demonstrated by measurement.
...
This thesis follows Approach B of prEN 50742. The choice reflects the role of the System Under Test as a component integrated into a larger industrial communication network, the situation addressed by Approach B. The methodology therefore uses IEC 62443-4-2 Component Requirements as the technical basis for the derived component capabilities rather than deriving an independent SRSL. The target assessment level is provisionally set to Component Security Level Capability (SL-C) 1, because the documented product context places the embedded device in a protected zone behind a firewall. % TODO-DECIDE: confirm target SL-C (1 or 2) with the supervisor and fix here
...
All acceptance criteria and all requirement verdicts reported in Chapter 4 are evaluated at this declared level. If the target level changes, the affected Component Requirements, test depth and verdicts must be reassessed.
...
\begin{itemize}
	\item \textbf{Requirement:} The derived component requirement (for example \gls{acr:RQ}-011) and its mapped \gls{acr:IEC} 62443-4-2 control (for example \gls{acr:CR} 2.1).
...
\subsection{State of the Art in Security Testing Frameworks}
\label{subsec:state_of_the_art_frameworks}
Security testing of industrial components can draw upon several established frameworks. These approaches vary significantly in their scope and focus, ranging from highly procedural IT security guidelines to abstract, compliance-oriented Operational Technology (OT) standards.
...
IEC 62443 Security Verification (Parts 4-1 and 4-2) defines the normative state of the art for industrial cybersecurity. Part 4-1 provides a comprehensive taxonomy for Security Verification and Validation (SVV), encompassing requirements-based testing, threat mitigation, vulnerability analysis, and penetration testing. Although exemplary in its structural completeness and alignment with compliance mandates, the standard remains fundamentally abstract: it prescribes \textit{what} must be verified but lacks concrete test cases or explicit tool specifications.

% TODO-CITATION: support or qualify the claim that the OWASP guides represent the industry standard.
The OWASP Web Security Testing Guide and IoT projects are widely used references for evaluating HTTP interfaces and Application Programming Interfaces (APIs). They provide actionable test scenarios for web services. However, they are primarily IT-focused and do not accommodate the deterministic OT fieldbus communication protocols (e.g., PROFINET, PROFIsafe) essential for this methodology. The OWASP IoT Security Testing Guide (ISTG) catalogues test cases across multiple hardware and software domains, including processing units (ISTG-PROC), memory (ISTG-MEM), firmware (encompassing installed instances [ISTG-FW{[}INST{]}] and update mechanisms [ISTG-FW{[}UPDT{]}]), data exchange services (ISTG-DES), internal interfaces such as I\textsuperscript{2}C and UART (ISTG-INT), as well as physical (ISTG-PHY), wireless (ISTG-WRLS), and user interfaces (ISTG-UI) \cite{owasp_istg}.
...
The company provides a manufacturer-specific security testing framework consisting of ten countermeasure-oriented instructions, denoted \gls{acr:CM}-001 to \gls{acr:CM}-010. Each instruction binds one countermeasure to one tool from Kali Linux, a Linux distribution that bundles preconfigured security testing tools, and to one acceptance rule. As noted in Section \ref{subsec:state_of_the_art_frameworks}, this framework is highly operational but organised around tools rather than derived requirements.
...
The applicability assessment produces one of four outcomes. A tool is applicable when the target exposes the assumed interface. A tool is not applicable when the assumed interface is known in advance to be absent from the device or topology, which is an applicability finding and not a product failure. A tool is blocked when an environmental constraint prevents its operation. A tool is \textbf{executed with negative result} when it is run and its negative result establishes the absence of the assumed interface on the device. This fourth outcome is epistemically distinct from not applicable: in the not-applicable case the absence is known in advance from documentation, whereas here the absence is demonstrated by the tool output and is itself evidence.
% TODO-OTHERFILE: apply the fourth applicability outcome to the applicability table and the test-mapping table in the implementation chapter, and update the counts of applicable / not applicable / blocked in that chapter, the summary and the abstract.
