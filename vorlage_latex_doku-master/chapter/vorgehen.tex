...
Every availability or robustness test declares its acceptance criterion against two distinct references, because such a requirement is verified from two different perspectives. The first is a specification-conformance criterion bounded by the device's specified operating envelope, such as the maximum number of concurrent connections permitted by the product specification; it verifies whether the intended function is preserved within the declared range and is the criterion applied to the availability aspect of RQ-007. Observations made outside that envelope do not, by themselves, support a Pass or Fail verdict against this first criterion and are recorded as overload observations. The second is an adversarial criterion applied to requirements that mandate resistance to a \gls{acr:DoS} condition, such as RQ-010 and its underlying \gls{acr:CR} 7.1, under which the component must maintain its essential functions when an attacker deliberately exhausts a resource. An adversary is not bound by the declared operating envelope, so a demonstrated and reproducible denial of an essential function is scored against this second criterion even when the triggering load lies outside the specified range. Here the declared envelope is treated as the potential weakness under evaluation, not as the acceptance threshold. The two criteria are reported separately so that a specification-conforming result and an adversarial \gls{acr:DoS} finding cannot be conflated. 
...
One rule governs how the absence of a function is scored, because absence can arise for two distinct reasons that must not be conflated. A tool that is Not Applicable is an artefact of the test instrument, the interface that this specific tool assumes is absent, so the tool yields no verdict, neither Pass nor Fail, and is merely removed from the evidence set of the requirement. A capability that the device demonstrably lacks is an artefact of the device, and it is scored Fail against the derived requirement whenever the executed evidence positively demonstrates that the mandated capability is absent or deviates from the obligation. The decisive criterion is therefore not whether a function was found, but whether its absence was positively demonstrated by the evidence. Observing that one possible realisation of a function is absent is not, on its own, a demonstration that the requirement's mandated capability is absent, unless no alternative realisation remains and the acceptance condition is thereby unmet. Consequently, a Not Applicable tool never lowers a requirement to Fail. The requirement is assessed on the remaining evidence and defaults to Not Assessed where that evidence neither demonstrates the capability nor demonstrates its absence. 
