...
\begin{table}[H]
	...
	\end{tabularx}
\end{table}
%TODO: The security categories and their requirement assignment reproduce the structure used in Chapter \ref{chap:implementation_results}. Confirm that this assignment matches the final results chapter, and re-check each SVV category label against Section \ref{subsec:iec_62443} rather than adopting it unchanged from the working notes.
...
%TODO überarbeiten welche Formeln sinnvollsind
Four Key Performance Indicators evaluate the security posture of the gateway and the quality of the test execution. These four indicators are defined for this methodology. They follow the ratio-based style of test-process metrics \cite[p. 145]{Witte2019}, but their number and their exact definition are specific to this work and are not prescribed by a standard. They are computed over the \textit{applicable} requirements, so that documented exclusions do not skew the result. Where a large part of the catalogue is specified but not executed, the indicators are reported over the performed subset, and the number of executed cases is stated alongside the ratio.
