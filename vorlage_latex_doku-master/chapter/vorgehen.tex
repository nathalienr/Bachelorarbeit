...
Building on the theoretical foundations established in \autoref{chap:fundamentals}, this chapter develops the security-testing methodology used in this thesis. The methodology translates the machine-level cybersecurity obligations of Regulation (EU) 2023/1230 into derived, component-level requirements and a traceable, repeatable engineering procedure. The chapter follows the derivation chain that forms the scientific core of the methodology.

\autoref{sec:research_design} defines the research design, the research questions, and the constraints. \autoref{sec:problem_context} describes the initial situation, the regulatory allocation of responsibility, and the methodological scope. \autoref{sec:requirements_analysis} explains how the fourteen security requirements are derived from the regulation and how each requirement is mapped to the affected assets, the relevant threats, and the security functions that address them. These links are consolidated into a single traceability chain. \autoref{sec:approach_selection} selects the verification techniques and assesses the applicability of the company framework. \autoref{sec:test_derivation} defines the development of the individual test cases. \autoref{sec:test_environment} defines the test environment and the three network access variants. \autoref{sec:evaluation_metrics} defines the verification and assessment model that converts test outcomes into requirement verdicts.
...
\begin{table}[H] 
	...
	\begin{tabularx}{\textwidth}{X X l} 
		...
		\midrule 
		Identification and asset discovery & Authentication (Identity verification) & \gls{acr:FR} 1 \\ 
		\addlinespace 
		...
		\addlinespace 
		Authentication and access control & Authentication, Authorisation and Access Control & \gls{acr:FR} 1, \gls{acr:FR} 2 \\ 
		\addlinespace 
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption[NV1, direct access]{NV1, direct access (Own illustration)}
	\label{fig:nv1}
\end{figure}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption[NV2, passive observation]{NV2, passive observation (Own illustration)}
	\label{fig:nv2}
\end{figure}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption[NV3, active in-path manipulation]{NV3, active in-path manipulation (Own illustration)}
	\label{fig:nv3}
\end{figure}
