...
\begin{table}[H] 
	...
	\end{tabularx} 
\end{table}
%TODO (Bonus) The applicability assessment correctly marks CM-002 "not applicable" because the flat lab topology lacks a zone/conduit model — but this raises an external-validity question the thesis doesn't address: how representative is a flat, single-switch lab network of a real customer deployment, where IEC 62443-3-3 zone segmentation would typically be present? A short paragraph acknowledging this as a threat to validity would strengthen the discussion of results.
...
%TODO was macht Nikto eigentlich? Debug interfaces identifizieren? eigentlich sollte das gerät keine debug interfaces haben, was bedeuten dann die findings?
...
Annex III, Section 1.1.9 requires that identification information of safety-relevant installed software be available at all times (RQ-007). Section 1.2.1 requires that the control system withstand reasonably foreseeable malicious attempts that could lead to a hazardous situation (RQ-010). %TODO geht es bei den beiden requirments nicht allgemein darun, dass das gerät, also das srio verfügbar bleibt auch bei dos etc.? und dass ich es hier eben nur getestet habe über die http verbindung?
...
%TODO Fault-state observation follow-up / disclosure process is left unresolved. Section 4.3.5 reports that structured HTTP fuzzing drove the SRIO into a visible device fault state that "had not fully cleared after the test was aborted" and explicitly states this "warrants closer review by the development". Given that IEC 62443-4-1 formally includes a Defect Management (DM) practice for exactly this scenario, and the thesis's own governance model (Appendix C, RACI matrix) assigns Software/Firmware Development a consulted role for findings, the thesis would be strengthened by explicitly stating whether this finding was escalated through that internal process, and by discussing responsible-disclosure/incident-handling implications of an unresolved fault state on a safety device — this is currently a gap between the empirical finding and the governance framework the thesis itself defines
...
%TODO Target Security Level Capability (SL-C) is never stated for the SRIO. Section 2.4.2 explains that Approach B requires components to meet IEC 62443-4-2 requirements at "a specified Security Level Capability (typically SL-C 1 or SL-C 2)", but the thesis never states which SL-C was actually targeted for the fourteen derived requirements applied to the SRIO. This materially affects how the two Fail verdicts (RQ-003, RQ-012) should be interpreted — a Fail against SL-C 1 is a different severity finding than against SL-C 2. Clarifying the chosen target level (and justifying it from the SUT's risk context) would close a real methodological gap. --> Verweis {subsec:derivation_mapping}
...
%TODO The Discussion of Results (§4.5.1) is a genuine strength — it explicitly distinguishes why a "Not Assessed" verdict occurs for different requirements (induced-event-not-triggered vs. access-position-never-established), which prevents the conservative aggregation rule from being misread as a blanket weakness. I found no circular reasoning or unsupported leaps between evidence and conclusions in Chapter 4; the "Fail" verdicts for RQ-003/RQ-012 are well-grounded in Table 4.6's tick-counter/log-deletion evidence.
