...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{l X X}
		...
		\midrule
		Identification and asset discovery & RQ-001, RQ-004, RQ-006, RQ-007, RQ-013 & Nmap (CM-001), Nikto (CM-007); \gls{acr:IoT}-Core inventory reads \\
		\addlinespace
		Communication security & RQ-001, RQ-002, RQ-014 & SSLscan (RQ-014 only) \\
		\addlinespace
		Logging and traceability & RQ-003, RQ-008, RQ-009, RQ-012 & \gls{acr:IoT}-Core error log, systick \\
		\addlinespace
...
An earlier, exploratory \gls{acr:TCP} scan used a window scan (\texttt{-sW}) at the \texttt{-T5} timing template instead of a SYN scan and reported numerous additional open ports that did not reappear in either \texttt{-sS} run. Because this exploratory scan differed in scan technique as well as in timing, the additional ports are attributable to the window scan's known unreliability against a non-standard embedded \gls{acr:TCP} stack rather than to scan timing alone; this exploratory result is not used as evidence, and the reproduced \texttt{-sS} result above is reported instead. A dedicated SYN scan at the \texttt{-T5} timing was not performed, so whether timing alone affects the device's responses remains an open item.
...
The absence of \gls{acr:TLS} is a device characteristic, not a failed \gls{acr:TLS} test. It is relevant to RQ-014 as a defence-in-depth observation.
...
The connection-hold procedure was executed twice against the identification interface. The first attempt did not reproduce the intended condition: its job listing shows two suspended processes left over from an earlier activity rather than the intended connection-holding processes, and the subsequent request returned \gls{acr:HTTP} status 200 within 0.25 seconds instead of timing out. This attempt is treated as a procedural execution failure and is not used as evidence. The procedure was repeated; the second job listing shows two connection-holding processes running as intended. While both connections were held, a third request to the identification endpoint did not receive a response within the five-second timeout. After one held connection was released, the same request again returned \gls{acr:HTTP} status 200. The evidence from this second run therefore demonstrates the intended connection-hold condition and recovery of the endpoint after a slot became available. The third request was made while the two permitted concurrent connections were already occupied and is consequently an outside-envelope overload observation, not a direct test of the specified operating condition. The activity remains \textbf{Not Assessed} for the requirement because it exercised only one endpoint and does not establish availability of all required identification information under every permitted operating condition.
...
This category covers the robustness aspect of RQ-010 under malformed input, with the availability asset H. The abstract catalogue contains the protocol-fuzzing scenario for the fieldbus-discovery, safety-frame, and web-interface inputs. The fieldbus-discovery and safety-frame fuzzing require the \gls{acr:NV2} or \gls{acr:NV3} position and were designed but not executed. The web-interface fuzzing executed under \gls{acr:NV1} is reported below.
...
A fuzzing test was run with boofuzz against the \gls{acr:HTTP} service on port 80. The saved script fuzzes the request line of an \gls{acr:HTTP} GET request. The boofuzz session databases are stored as evidence (Appendix \ref{app:cmd_robustness}). %TODO-VERIFY: the timing detail below (approx. 41 s recovery) could not be independently confirmed from a plain-text export in Tests_BA; verify against the session database content or attach a text export as a stored artefact.
...
During the run, the device's BF and SF indicator LEDs (bus-fault and system-fault indicators, respectively) were both observed lit red; the SF indicator turned off as the run continued. After the test was aborted, the BF indicator was off while the SF indicator and a third indicator, labelled FS, showed red. A manual read of the error log taken after the run showed one additional \texttt{FATAL}-level entry and three additional \texttt{ERROR}-level entries that were not present in the baseline of Section \ref{subsec:res_evidence}. This reading was not captured to a stored file and is therefore reported as an unscored observation rather than as evidence. %TODO-VERIFY: capture a screenshot or an exported error-log read of this fault indication as a stored artefact in a repeat run.
Independently of the specific error codes involved, which are not interpreted here, the indicator and error-log behaviour together show that the device entered a fault state as a consequence of the \gls{acr:HTTP}-level fuzzing. Because the affected indicators concern safety monitoring (asset E), this observation is also relevant to the domain-separation obligation of RQ-001 (Section \ref{subsec:res_attack_surface}, asset F): it suggests, without yet proving, that the disruption is not confined to the non-safety interface.
...
%TODO entsprechend anpassen
This section closes the chapter by evaluating the methodology itself, as distinct from the requirement verdicts of Section \ref{sec:requirement_verification} and their interpretation in Section \ref{sec:discussion}. Section \ref{subsec:objective_validation_sub} validates the methodology against the thesis objective, following the evaluation activity of the Design Science Research process. Section \ref{subsec:threats_to_validity} examines the reliability of the findings and the threats to construct, internal, and external validity that bound them.
