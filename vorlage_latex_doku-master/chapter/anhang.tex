...
Within the scope of this thesis, tools based on Artificial Intelligence (\gls{acr:AI})\index{Artificial Intelligence} were used. Table \ref{tab:anhang_uebersicht_KI_werkzeuge} provides an overview of the tools used and their respective purpose of application.
...
% TODO-RENAME: RQ-0xx (security requirements, defined here) collides with RQ1-RQ3
% (research questions). If renamed to SR-0xx, this appendix is the normative source and
% must be changed together with Chapters 3, 4 and 5 in one coordinated pass.

This appendix provides the complete, requirement-by-requirement documentation underlying the derivation methodology described in Section \ref{subsec:derivation_mapping}. It contains 14 requirements derived from obligations addressed to machinery and related products under \hyperref[subsec:eu_machinery_regulation]{\textbf{Regulation (\gls{acr:EU}) 2023/1230}}. They are expressed here as derived component-level requirements for the System Under Test for verification purposes; they do not state that the Regulation applies directly to the component. This allocation is consistent with \autoref{sec:regulatory_applicability}. The requirements are mapped to \hyperref[subsec:pren_50742]{\textbf{prEN 50742}} and, subsequently, to \hyperref[subsec:iec_62443]{\textbf{\gls{acr:IEC} 62443-4-2}} Component Requirements (\gls{acr:CR}s), Requirement Enhancements (\gls{acr:RE}s), and component-type-specific instantiations such as Embedded Device Requirements (\gls{acr:EDR}s). The three tables correspond to the three derivation stages and share a common requirement identifier (\gls{acr:RQ}-XXX) to preserve traceability across all stages.

\section{Security Requirements Derived from Regulation (EU) 2023/1230}
\label{app:sec:mvo_requirements}

Table \ref{tab:mvo-requirements} lists the 14 derived component-level requirements originating from Annex III, Sections 1.1.9 and 1.2.1 of Regulation (\gls{acr:EU}) 2023/1230. Each entry records the originating source clause, the requirement statement, the assigned Security Objective, the Requirement Type, and a rationale explaining the derivation from the regulatory text.
...
% RQ-014 gloss: access to tracing-log data is restricted to a justified conformity request from a competent national authority.

\section{Mapping to prEN 50742}
\label{app:sec:mapping_pren50742}

Table \ref{tab:pren50742-mapping} maps the 14 requirements of Table \ref{tab:mvo-requirements} onto the corresponding clauses of \hyperref[subsec:pren_50742]{\textbf{prEN 50742}} (\emph{Safety of machinery - Protection against corruption}). The mapping types are used as follows: \textbf{Partial} indicates that the clause addresses the obligation but does not fully cover it; \textbf{Supporting} indicates that the clause contributes context or a prerequisite but is not the operative requirement; and \textbf{No mapping} indicates that the draft standard explicitly excludes or does not address the obligation.
\begin{longtable}{|>{\bfseries}p{1cm}|p{2.5cm}|p{9cm}|>{\centering\arraybackslash}p{2cm}|}
	...
	\hline
	\textbf{Requirement ID} & \textbf{prEN 50742 clause} & \textbf{prEN 50742 requirement/topic} & \textbf{Mapping type} \\
	\hline
	\endfirsthead
...
Table \ref{tab:mvo-pren-iec62443-mapping} extends the derivation chain from \hyperref[subsec:pren_50742]{\textbf{prEN 50742}} to \gls{acr:IEC} 62443-4-2. Where no \hyperref[subsec:pren_50742]{\textbf{prEN 50742}} clause is available, the mapping proceeds directly from \hyperref[subsec:eu_machinery_regulation]{\textbf{Regulation (\gls{acr:EU}) 2023/1230}}. The mapped Component Requirements are to be satisfied at the target Component Security Level Capability declared in Section \ref{subsec:derivation_mapping}; applicable Requirement Enhancements are included accordingly. \gls{acr:IEC} 62443-4-2 organises its Component Requirements under the seven Foundational Requirements introduced in Section \ref{subsec:iec_62443}.

\footnotesize
\begin{longtable}{|>{\bfseries}p{1.0cm}|p{0.9cm}|p{2.4cm}|p{2.4cm}|p{4.8cm}|p{1.8cm}|}
	\caption{Mapping of Machinery Regulation-derived security requirements via prEN 50742 and the IEC 62443 Foundational Requirements to IEC 62443-4-2 component requirements} \label{tab:mvo-pren-iec62443-mapping} \\
	\hline
	\textbf{Requirement ID} & \textbf{FR} & \textbf{CR/RE/EDR} & \textbf{IEC 62443-4-2 topic} & \textbf{Mapping rationale} & \textbf{Type} \\
	\hline
	\endfirsthead
	...
	CR 3.1 covers the integrity of the information transmitted over such an interface, while EDR 3.11 covers protection of the interface hardware itself against unauthorised physical access and corruption. &
	Partial \\
	\hline
	...
	CR 2.1 (authorisation enforcement) is the primary control restricting who may access the log and for what purpose, and CR 4.1 (confidentiality of information at rest for which read authorisation is explicitly required). CR 3.9 additionally protects the log against tampering and unauthorised deletion. CR 6.1 (ability to access event logs) is included only as a supporting control, since it ensures that an already-authorised party can retrieve the log on request. &
...
The lifecycle demonstrates that cybersecurity is not limited to a single verification activity prior to product release. Instead, security is addressed throughout the development and operational lifecycle \cite[pp. 5]{NiemannPuls2023} \cite[pp. 68]{Wurm2022}. The process begins with the definition of the scope and security objectives. A Threat Analysis and Risk Assessment (\gls{acr:TARA}), as described in Section \ref{subsec:tara}, is performed to identify and assess relevant cybersecurity risks and to determine the required security level. Based on the assessment results, detailed countermeasures are derived and incorporated into the security architecture. The resulting architectural constraints provide input to the development phase, during which the specified security measures are implemented \cite[pp. 8]{NiemannPuls2023}. 
...
\paragraph{RACI Governance of the Verification Lifecycle}
\label{sec:raci_governance}

% TODO-VERIFY: confirm the practice designation used in Chapters 2 and 3 and match it here.

To ensure strict organisational accountability, responsibility for the testing phases is governed through a Responsible-Accountable-Consulted-Informed (\gls{acr:RACI}) matrix. This established management method is described as ensuring exactly one role is accountable for a given activity \cite{cybersierra_security_raci_2026, su13041806}. In this scheme, the responsible role performs the activity, the accountable role owns and approves the outcome, consulted roles provide input before a decision, and informed roles are notified of the result. Six generic roles are distinguished for this purpose \cite[p. 6]{Schmidt2026CybersecurityTestProcess}. % TODO-VERIFY: confirm that the cited internal source is available to the intended readers and is not publicly available.
...
This appendix lists the tool commands used for the activities executed under the NV1 network position, as reported in Section \ref{sec:executed_tests}. The placeholder \texttt{\$SRIO} denotes the IP address of the Safe Remote I/O, \texttt{192.168.0.2}.
All commands were executed from the Kali Linux test system under NV1 in the isolated laboratory environment described in Section \ref{subsec:realized_environment}. The commands are reproduced as run, and the file names below are the names of the stored evidence artefacts produced during the test campaign.
\begin{verbatim}
...
% TODO-VERIFY: confirm the cm001_srio_tcp and cm001_srio_udp file names against the stored evidence; the naming scheme differs from the cm_rq001-02_srio.html artefact.
\begin{verbatim}
	
	# Full TCP scan with service detection
	sudo nmap -sS -sV -p- -T3 -oA cm001_srio_tcp $SRIO
	# UDP top-ports scan
	sudo nmap -sU --top-ports 200 -oA cm001_srio_udp $SRIO
	# Web-server scan
...
% TODO-VERIFY: confirm cm_rq006-03_sslscan.txt against the stored evidence; the identifier does not match the RQ-004/RQ-014 requirements supported by this activity in Chapter 4.
\begin{verbatim}
...
\begin{verbatim}
	...
	# remove power for a cold restart; induce the second short-circuit; repeat the reads.
\end{verbatim}

\section{Availability under Connection Load}
\label{app:cmd_availability}
% TODO-VERIFY: confirm cmrq010_siege.log and cmrq010_3connections_siege.log against the stored evidence; the names omit the underscore used in the other appendix artefacts.
\begin{verbatim}
	...
	# a further request that is expected to time out.
\end{verbatim}
The connection-hold procedure was performed manually. No command listing or stored artefact documents that the connection slots were occupied; this limitation is reported in Section \ref{subsec:res_availability}.
